...
public class ImageUtils {
     ...
     * @throws IOException 
     */
    public static void addImageWatermark(String inputImage,String markImage,float alpha) throws IOException{
    	File file=new File(inputImage);
    	BufferedImage bufferedImage=ImageIO.read(file);
    	Thumbnails.of(bufferedImage).size(bufferedImage.getWidth(),bufferedImage.getWidth())
    	.watermark(Positions.BOTTOM_RIGHT,ImageIO.read(new File(markImage)),alpha)  
    	.outputQuality(alpha).toFile(inputImage);
	 ...
	 * @param targetAddr 目标文件地址
	 * @throws IOException
	 * </pre>
	 */
    public static void compressImage(File file, int width, int height, String filename,
			String targetAddr) throws IOException {
    	...
    	if (w*height>=h*width) {
			builder.height(height).sourceRegion(Positions.CENTER, h*width/height,h).outputFormat(fileType).toFile(targetAddr +File.separator+  filename);
		}else {
			builder.height(width).sourceRegion(Positions.CENTER, w,w*height/width).outputFormat(fileType).toFile(targetAddr +File.separator+  filename);
		}
	}
    
	public final static String getFileType(File file) {
		if (null==file) {
			throw new IllegalArgumentException("file not empty");
		}
		String filaName = file.getName();
		return filaName.substring(filaName.lastIndexOf(".") + 1);
	}
	 ...
	 * @param targetAddr 目标文件地址
	 * @throws IOException
	 *    targetAddr /roor/image
	 * </pre>
	 */
	public static void compressImage(FileInputStream stream, int width, int height, String filename, String fileType,
			String targetAddr) throws IOException {
		Thumbnails.of(stream).size(width, height).toFile(targetAddr +File.separator+  filename + "." + fileType);
	}
	
	/**
	 * <pre>
	 * @param stream 输入流
	 * @param factor 质量因子 质量因子 参数是浮点数，0-1之间
	 * @param scale  参数是浮点数,大于1表示放大，小于1表示缩小
	 * @param filename 文件名称
	 * @param fileType 文件类型
	 * @param targetAddr 目标文件路径
	 * </pre>
	 * @throws IOException 
	 */
	public static void compressQuality(FileInputStream stream, double factor,double scale, String filename, String fileType,
			String targetAddr) throws IOException{
		Thumbnails.of(stream).scale(scale).outputQuality(factor).outputFormat(fileType).toFile(targetAddr+File.separator+filename);
	}
	
	/**
	 * <pre>
	 * @param file 文件
	 * @param factor 质量因子 参数是浮点数，0-1之间
	 * @param scale  参数是浮点数,大于1表示放大，小于1表示缩小
	 * @param filename 文件名称
	 * @param fileType 文件类型
	 * @param targetAddr 目标文件路径
	 ...
	 * @throws IOException 
	 */
	public static void compressQuality(File file, double factor,double scale, String filename, String fileType,
			String targetAddr) throws IOException{
		Thumbnails.of(file).scale(scale).outputQuality(factor).outputFormat(fileType).toFile(targetAddr+File.separator+filename);
	}
	...
	public static String[] getImagePath(String path) {
		...
		if(img!=null && img.length ==2 && img[1].indexOf(".")>0){
			String[] suf  = StringUtils.split(img[1], ".");
			if(suf !=null && suf.length ==2){
				img[1] = suf[0];
				target[2]=suf[1];
			}
		}
	...
	public static String getImagePrefix(String url){
		if (StringUtils.isNotBlank(url)) {
			return url.replace("."+(url.substring(url.lastIndexOf(".")+1)), "").trim();
		}
		return null;
	}
     ...
     * </pre>
     */
	private final static boolean validImageType(String type) {
		if (StringUtils.isBlank(type)) {
			return false;
		}
		if (ImageUtils.IMAGE_TYPE_GIF.equalsIgnoreCase(type)) {
			return true;
		} else if (ImageUtils.IMAGE_TYPE_JPEG.equals(type)) {
			return true;
		} else if (ImageUtils.IMAGE_TYPE_JPG.equals(type)) {
			return true;
		} else if (ImageUtils.IMAGE_TYPE_PNG.equals(type)) {
			return true;
		} else {
			return false;
		}
